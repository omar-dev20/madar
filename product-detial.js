    ...
    try {
        ...
        if (btnAddToCart) {
            btnAddToCart.addEventListener('click', function() {
                ...
                let existingProductIndex = cart.findIndex(item => String(item.id) === String(get.id));
                if (existingProductIndex !== -1) {
                    cart[existingProductIndex].qty = String(Number(cart[existingProductIndex].qty) + Number(productToCart.qty));
                } else {
                ...
                if (updateCount) {
                    updateCount.textContent = cart.length;
                }
            });
        }
...
async function loadSimilarProducts(category, currentProductId) {
    ...
    try {
        let response = await fetch(`https://dummyjson.com/products/category/${category}?limit=5`);
